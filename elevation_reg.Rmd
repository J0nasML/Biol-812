...
shrubpoints <- st_as_sf(rcstats, coords = c("lat", "long"), crs = 4326) #4326 is the WGS coordinate system
st_crs(shrubpoints) #our data is now georeferenced
UTMshrubs <- st_transform(shrubpoints, CRS("+init=epsg:2455")) # I can't get this code to transform CRS's correctly
...
shrubbuffer <- raster::extract(DEM,
                      shrubpoints,
                      buffer = 100,
                      fun = mean,
...
?ggmap
#elevplot <- bwplot(DEM ~ site,                   ##no need for this right now
                   data = data.frame(shrubbuffer), 
                   notch = TRUE, 
                   pch = "|", 
                   fill = "grey", 
                   box.ratio = 0.25, 
                   par.settings = list(
                     box.rectangle = list(
                       col = c("red","blue", "green", "magenta", "cyan", "magenta"))),
                   ylab = "elevation (m)", 
                   main="Elevation",
                   scales = list(x = list(labels = c("Site 1",
                                                     "Site 2",
                                                     "Site 3",
                                                     "Site 4",
                                                     "Site 5",
