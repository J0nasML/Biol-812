...
car::symbox(~MeanMeasure,data=tempdat)#symobox suggests a log transformation which may a consideration for future analysis. 
```
...
#There are cooks distance value over the standered threshold of 2. This points exhibits influence over the model and should not be included. 
car::outlierTest(tempdatanooutfit1)#Can use outlier test to double check
car::symbox(~MeanMeasure,data=tempdatanoout)#symobox is inconclusive 
...
bc<-boxCox(moistfit1)
(bestlam<-bc$x[which(bc$y==max(bc$y))])#boxcox suggests square rooting the data 
```
```{r}
